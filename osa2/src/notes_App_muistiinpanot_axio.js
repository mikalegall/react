  ...
  return () => {
    ...
    noteService // Murupolussa ./services/muistiinpanot.js
      ...
      .catch(virheilmoitus => { // Mikäli promise menee tilaan "response_resolve_result_rejected" käsitellään virheilmoitus 
        ...
        this.setState({
          error: `Muistiinpano sisällöltään '${note.content}' on jo valitettavasti poistettu palvelimelta`,
          notes: this.state.notes.filter(n => n.id !== id) }) // Filtteröidään kaikki muut paitsi tämän virheen antaneen muistiinpanon ID näytettäväksi
      })
      setTimeout(() => {
